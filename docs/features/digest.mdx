...
Status: The Digest feature is currently in active development. Stay tuned for more updates and improvements.
...
1. For each notification, you can configure each channel at the `Configure` tab.

   In this channel configuration, you can see following `Delivery Options`:
   ...
   The following shows a sample of an `Email` channel configuration.
